...
Since \code{train} always stores a copy of the training data as part of the \code{"train"} object in a component called \code{trainingData} (e.g., \code{boston.xgb.tune\$trainingData}) there is no need to supply the training data in the call to \code{partial}. However, this is not the case when using the \pkg{xgboost} package directly. For example, the following code fits the same model using the \code{xgboost} function.
\begin{example}
