...
The \code{plotPartial} function can be used for displaying more advanced PDPs; it operates on objects of class \code{"partial"} and has many useful plotting options (e.g., the option to add a LOESS smooth). Of course, since the default output produced by \code{partial} is still a data frame, the user can use any plotting package he/she desires to visualize the results---\CRANpkg{ggplot2} \citep{ggplot2-pkg}, for instance.
