...
boston.xgb <- train(x = data.matrix(subset(boston, select = -cmedv)),
                    ...
                    tuneLength = 10)
\end{example}
The optimal model had a cross-validated $R^2$ of $0.902$ which was achieved using \code{nrounds = 100}, \code{max\_depth = 5}, \code{eta = 0.3}, \code{gamma = 0}, \code{colsample\_bytree = 0.8}, \code{min\_child\_weight = 1}, and \code{subsample = 0.9444444}. The following snippet of code computes the partial dependence of \code{cmedv} on both \code{rm} and \code{lstat}, individually and together, and then uses \code{grid.arrange} to display all three PDPs on the same page. The results are displayed in Figure~\ref{fig:boston_xgb}.
\begin{example}
