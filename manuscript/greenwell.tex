...
By default, \code{partial} constructs PDPs for regression on the response scale, and PDPs for classification on the logit\footnote{Add footnote here!} scale. However, in some cases it may be more informative to view PDPs on the probability scale.

It is possible to provide your own prediction function to via the \code{pred.fun} argument in the call to \code{partial}. This is useful if, for example, the user requires PDPs for classification problems on the probability scale.

By default, this function computes Equation~\eqref{eqn:pdf} and requires two arguments: \code{object} and \code{newdata}. This makes it possible to replace the sample mean in Equation~\eqref{eqn:pdf} with the median, or the logit in Equation~\eqref{eqn:avg-logit} with the predicted probaility of a particular class.

The prediction function provided to \code{pred.fun} must have two specific arguments: \code{object} and \code{newdata}. Additionally, the function must return a single value---typically the mean of the predicted values.

For regression problems, the default prediction function is given by
\begin{example}
pred.fun <- function(object, newdata) {
  mean(predict(object, newdata), na.rm = TRUE)
}
\end{example}
which corresponds to Equation~\eqref{eqn:pdf}.

To obtain PDPs on the probability scale we just need to write a function that extracts the predicted class probabilities of interest and the averages the result. The snippet of code below does uses the fitted SVM from the previous example to extract the predicted probability of belonging to the setosa species.
\begin{example}
...
\end{example}
Next, we simply pass this function via the \code{pred.fun} argument in the call to \code{partial}. This result is displayed in Figure~\ref{fig:partial_iris_prob}.
\begin{example}
# Figure 8
...
\begin{figure}[!htbp]
  ...
  \caption{Partial dependence of \code{Species} on \code{Petal.Width} and \code{Petal.Length} plotted on the probability scale; in this case, the probabiltity of belonging to the setosa species.}
  \label{fig:partial_iris_prob}
\end{figure}

This argument makes \code{partial} quite flexible. For instance, it is possible to obtain the indivudal conditional expectation (ICE) curves described in \citet{goldstein-peeking-2015} and made available in the \pkg{ICEbox} package. To obtain ICE curves, the user must supply \code{pred.fun} with a function that simply returns the desired type of prediction for each observation. This is demonstarted for the Boston housing example in the code snippet below.
\begin{example}
...
\end{example}
Whenever returns a data frame that also inherits from class \code{"partial.ice"}. The only difference is the addition of a new column called \code{"obs"} which indicates which observation a curve belongs to; in this example there is one for each unique observation. To further illustrate, the code below constructs ICE curves for specific quantiles of the predictions. The results, along with the typical PDP for \code{lstat}, are displayed in Figure~\ref{fig:partial_boston_ice}.
\begin{example}
...
\begin{figure}[!htbp]
  ...
  \caption{ICE plots and PDP for the Boston housing data. \textit{Left}: ICE curves. \textit{Middle}: Specific quantiles of ICE curves. \textit{Right}: Standard PDP.}
