...
\end{example}
Now, to run \code{partial} in parallel, all we have to do is invoke the \code{.parallel = TRUE} option and the rest is taken care of by the internal call to \pkg{plyr} the parallel backend we loaded. This is illustrated in following snippet of code which obtains the partial dependence of median home value on $\boldsymbol{z}_s = \left\{\code{rm}, \code{ptratio}, \code{chas}\right\} in parallel. The result is displayed in Figure~\ref{fig:partial_par}.
\begin{example}
partial(boston.rf, pred.var = c("rm", "ptratio", "chas"), grid.resolution = 20, 
        chull = TRUE, .parallel = TRUE, plot = TRUE)
\end{example}
