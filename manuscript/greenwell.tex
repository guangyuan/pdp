  ...
  \begin{tabular}{p{4cm}ll}
    \toprule
      ...
      Boosted decision trees    & \pkg{gbm}      & \code{"gbm"} \\
                                & \CRANpkg{xgboost} \citep{xgboost-pkg} & \code{"xgb.Booster"}
...
Finally, for further illustration, we will compare the random forest results with those obtained from gradient boosting; for this, we shal use the popular \pkg{xgboost} package. The example below, uses \pkg{xgboost} to model the boston housing data. The tuning parameters were predetermined using 5-fold cross-validation.
\begin{example}
set.seed(102)
...
To use \code{partial} with the \code{"xgb.Booster"} class, we need to supply the original training data (minus the response!) in the call to \code{partial}:
\begin{example}
