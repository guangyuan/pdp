...
The code chunk below uses \pkg{caret} to train an XGBoost model over a grid of tuning values using 5-fold cross-validation.
\begin{example}
...
boston.xgb <- train(x = data.matrix(subset(boston, select = -cmedv)),
                    ...
                    tuneGrid = xgb.grid)
\end{example}
The model with the smallest root-mean-squared error (RMSE) had \code{nrounds = 2000}, \code{max\_depth = 3}, \code{eta = 0.01}, and achieved a cross-validated RMSE and $R^2$ of 2.819 and 0.905, respectively. The following snippet of code computes the partial dependence of \code{cmedv} on both \code{rm} and \code{lstat}, individually and together, and then uses \code{grid.arrange} to display al three PDPs on the same page. The results are displayed in Figure~\ref{fig:boston_xgb}.
\begin{example}
p1 <- partial(boston.xgb, pred.var = "rm", plot = T, rug = T)
p2 <- partial(boston.xgb, pred.var = "lstat", plot = T, rug = T)
p3 <- partial(boston.xgb, pred.var = c("lstat", "rm"), plot = T, chull = T),
grid.arrange(
  p1,  # Figure 8 (left)
  p2,  # Figure 8 (middle)
  p3,  # Figure 8 (right)
