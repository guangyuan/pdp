...
\strong{Note:} as mentioned above, \pkg{pdp} relies on \pkg{lattice} for its graphics. \pkg{lattice} itself is built on top of \CRANpkg{grid} \citep{grid-pkg}. \pkg{grid} graphics behave a little differently than traditional R graphics and two points are worth making:
\begin{enumerate}
  \item \pkg{lattice} functions create a \code{"trellis"} object, but does not display it; the \code{print} method produces the actual display. The result is automatically printed when using these functions in the command line, but in \code{source} or inside a function, an explicit \code{print} statement is required to display the resulting graph.
...
\end{enumerate}
A consequence of second point is that the \code{par} function can not be used to control the layout of multiple \pkg{lattice} (and hence \pkg{pdp}) plots. Simple solutions are available in packages \CRANpkg{latticeExtra} and \CRANpkg{gridExtra}. For convenience, \pkg{pdp} imports the \code{grid.arrange} function from \pkg{gridExtra} which makes it easy to display multiple \pkg{pdp} plots on a single page.
...
The \code{partial} function also supports object of class \code{"train"} produced using the \code{train} function from the well-known \CRANpkg{caret} package \citep{caret-pkg}. This means that \code{partial} can be used with any classification or regression model that \code{caret} supports; see \url{http://topepo.github.io/caret/available-models.html} for a current list of supported models. An example is given in Section~?.
...
plotPartial(pd, contour = TRUE, col.regions = rwb)  # Figure 3 (middle)
...
The code chunk below uses \pkg{caret} to train an XGBoost model of a grid of tuning values using 5-fold cross-validation.
\begin{example}
...
boston.xgb.tune <- train(x = data.matrix(subset(boston, select = -cmedv)),
                         ...
                         tuneGrid = xgb.grid)
\end{example}
The final model achieves a cross-validated RMSE and $R^2$ of 2.819 and 0.905, respectively. The following snippet of code computes the partial dependence of \code{cmedv} on both \code{rm} and \code{lstat}, individually and together. The results are displayed in Figure~\ref{fig:boston_xgb}.
\begin{example}
grid.arrange(
  # Figure 8 (left)
...
\end{example}
To use \code{partial} with the \code{"xgb.Booster"} class, we need to supply the original training data (minus the response) in the call to \code{partial}. The following snippet of code computes the partial dependence of \code{cmedv} on \code{rm} (plot not shown). \strong{Note:} while \code{xgboost} requires the training data to be an object of class \code{"matrix"}, \code{"dgCMatrix"}, or \code{"xgb.DMatrix"}, \code{partial} requires a \code{"data.frame"}.
\begin{example}
