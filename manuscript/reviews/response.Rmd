...
A few additional changes have been made to the package since the original submission. The Boston housing data no longer has to be imported from the `mlbench` package; it is loaded automatically with `pdp`. `pdp` now interfaces with the `caret` package. This is breifely discussed on page X and has been incorporated into the last example.

A few of the comments were related to the plotting behavior of `plotPartial`. I added a paragraph on page X that should help clarify many of these issues.
...
#### Response
Fixed. The text was wrapped appropriately.
