...
```
#### Response
This is simply a side effect of using `lattice` to produce the plots. If sourcing a script, then the plot needs to be explicitly printed (e.g., `print(plotPartial(pd))`). This has been explained in both the documentation and the manuscript in a **Note:** on 3.
...
```
#### Response
The `gbm` maintainer did contact me a while back and this has been fixed in `pdp` (version 0.3.0). The user will still have to supply `n.trees` in the call to `partial`. The test suite is being expanded, as suggested in a later comment, to help catch more errors like this.


#### Comment
Standard partial matching isn't implemented (e.g., `partial(..., type="r")`).
#### Response
Partial matching is now implemented, but only for the `type` argument. Thanks for the suggestion.
...
#### Response
Aggred! Unfortunately, some functions just don't store a copy of the training data. I have expanded the code to work harder, especially for `S4` methods, to inform the user to supply the training data when necessary. The documentation for `partial` has also been expanded to discuss this.
...
#### Response
The last column is now labeled `yhat`; which is less likely to be a predictor name. When one of the predictors is labeled `"y"` `randomForest::partialPlot` fails while `gbm::plot.gbm` does not. I think regardless of what the default column name is, there is always the possibility of this issue. This has been filed as an issue on the `pdp` GitHib page :
...
```
#### Response
The package was originally called `partial`, but at the time of its original submission this conflicted with a CRAN package of the same name (but with mixed lowercase/uppercase letetrs). So, I decided to change the name of the package to `pdp`; besides, I think `partial` and `plotPartial` work well together as function names. However, I did add a help page describing the package, main functions, etc. So, `?pdp` no longer fails. I am, however, not opposed to creating an alias to `partial` called `pdp` in the future.


#### Comment
`partial` should return the same value regardless of whether or not the `plot` argument is used?
#### Response
I dont think so, and this is the same behavior for the PDP functions in `randomForest` and `gbm` (not that `pdp` needs to immitate the behavior of these packages PDP functions). However, I did add the partial dependence data as an attribute to the returned `"trellis"` object (i.e., plot) so that the user can extract the data easily from that if necessary (e.g., if they do not like the plot and don't want to re-run partial). For example,
```{r}
library(pdp)
data(trees)
mod <- lm(Volume ~ ., data = trees)
...
```
For convenience, `pdp` now imports the `grid.extra` function and this has been incorporated into the manuscripts examples where appropriate.
...
#### Response
This is also true for the fitting functions in other package (e.g., `kernlab::ksvm`). As time goes on, the coverage will certainly be inncreased to make it easier on the user. For example, `partial` now works with `caret` which, by default, stores the taining data in a component called `trainingData`. I suspect that the package maintainers for `xgboost`  would be hesitent in storing a copy of the training data, especially since the package is often used for modeling large data sets; they probably do not want to store a copy of the data with the fitted model object for memory reasons.
...
#### Response
This is a good suggestion and is currently under development. In fact, the `gbm` maintainers have already contacted me about a breaking change caused by the next planned release of their package.
...
#### Response
Rather than using subfigures, which I think would look messy with all the code, I made quite an attempt to comment which pieces of code produce which figure---in addition to in the text. Also, in response to another reviewer's comments, the code which produces figures containing multiple plots is better described and demonstrated. See, for example, the section called Multi-predictor PDPs.
...
#### Response
I agree, and I have changed the behavior of `...` to refer to additional parameters to `stats::predict` (e.g., the `n.trees` argument in `predict.gbm`.). However, I'm not sure yet about the scale on which the predictions are plotted. For now, I am more comfortable leaving as the usual logit scale defined for PDPs (mostly so that they are defined the same whatever the number of class outcomes). But I am not opposed to making this (simple) change in the future. I have added this as a feature request: https://github.com/bgreenwell/pdp/issues/21.
...
#### Response
Good suggestion. I tweaked the example to run in both Windows and Unix-like systems. This is also reflected in the manuscript text.
